...
def make_train(config):
    ...
    if config["USE_OPTIMISTIC_RESETS"]:
        ...
        env_viz = BatchEnvWrapper(env_viz, num_envs=config["TEST_NUM_ENVS"])
    ...
    def train(rng):
        ...
        if config['ACTION_IN_OBS']:
            obs_shape = env.observation_space(env_params).shape[:-1] + (
            env.observation_space(env_params).shape[-1] + 1,)
        else:
