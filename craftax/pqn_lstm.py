...
def parse_args():
    ...
    parser.add_argument(
        "--run_name", type=str, default="pqn_lstm", help="Name of the run"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--NUM_STEPS", type=int, default=128, help="steps per environment in each update"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--Q_LAMBDA", action=argparse.BooleanOptionalAction, default=False
    )
    ...
    parser.add_argument(
        "--NORM_TYPE", type=str, default="layer_norm", help="layer_norm or batch_norm"
    )
    parser.add_argument(
        "--MEMORY_WINDOW",
        type=int,
    ...
    parser.add_argument(
        "--ADD_LAST_ACTION", type=bool, default=True, help="Add last action to input"
    )
        ...
        def _logging_step(runner_state, unused, logging_threads):
            ...
            def write_rnn_hstate(hstate, scalars, increment=0):
                ...
                for i in range(logging_threads):
                    out_filename_hstates = os.path.join(
                        run_out_path, "hstates_{}_{}.csv".format(increment, i)
                    )
                    temp_filename = os.path.join(run_out_path, "temp.csv")
                    np.savetxt(temp_filename, cell_state[:, i, :], delimiter=",")
            ...
            jax.debug.callback(
                write_rnn_hstate, hidden_states, log_array, runner_state[0].n_updates
            )
