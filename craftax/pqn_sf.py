...
def parse_args():
    ...
    parser.add_argument("--SF_DIM", type=int, default=256, help="SF dimension")
...
def make_train(config):
    ...
    def train(rng):
        ...
        if "pixel" in config["ENV_NAME"]:
            raise ValueError("Pixel obs not supported yet")
            is_symbolic = False
        else:
            is_symbolic = True
...
def run_pqn(config):
    ...
    def _save_network(rs_index, dir_name):
        multi_train_states = out["runner_state"][rs_index]
        multi_train_state = jax.tree_map(lambda x: x[0], multi_train_states)
        orbax_checkpointer = PyTreeCheckpointer()
        options = CheckpointManagerOptions(max_to_keep=1, create=True)
        path = os.path.join(wandb.run.dir, dir_name)
        checkpoint_manager = CheckpointManager(path, orbax_checkpointer, options)
        print(f"saved runner state to {path}")
        save_args = orbax_utils.save_args_from_target(multi_train_state)
